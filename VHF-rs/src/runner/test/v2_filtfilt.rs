...
fn filtfilt_v2_file_trivial_filter() {
    ...
    let filter_b = vec![1.]; // While scipy does reject this, this is the effective filter conceptually, which is an impulse filter.
...
/// Test by considering a lossy len-2-filter on filtfilt.
/// No decimation is yet checked.
#[test]
#[ignore = "incomplete test"]
fn filtfilt_v2_file_len2_lossy_filter() {
    let debug_vhf_total_len = 12 * VHF_MMAP_WINDOW_LEN;
    const PARAMS_STEP_BY: usize = VHF_MMAP_WINDOW_LEN.checked_sub(1).unwrap();
    // Ideally this should be derived from debug_vhf_conf, but we cannot due to circular
    // requirement; const to enforce at compile time.
    ...
    let filter_b = vec![1., f64::MIN_POSITIVE]; // Creates a lossy impulse filter of len 2. That aims to be as close
    // as possible to impulse.
    let decimation_factor = unsafe { NonZeroUsize::new_unchecked(1) };
    // WARN: There is currently no means of exposing generating of filtfilt via
    // `Config::with_config`.
    debug_vhf_conf.stream_fold = {
    ...
    if let StreamFoldOp::Map(Some(map_arg)) = &params_func.op {
        ...
        assert_eq!(
            map_arg.num_before_first_drop,
            filter_b.len().checked_sub(1).unwrap(),
            "The lossy impulse filter drops exactly 1 less than itself for file writing."
        );
    } else {
        panic!("func.op should have been Map(Some(_))!");
    }
...
/// Test by considering a specific filtfilt filter on period 2 signal.
/// No decimation is yet checked.
#[test]
#[ignore = "incomplete test"]
fn filtfilt_v2_file_len3_trivial_filter() {
    let debug_vhf_total_len = 12 * VHF_MMAP_WINDOW_LEN;
    const PARAMS_STEP_BY: usize = VHF_MMAP_WINDOW_LEN.checked_sub(1).unwrap();
    // Ideally this should be derived from debug_vhf_conf, but we cannot due to circular
    // requirement; const to enforce at compile time.
    ...
    // This filter is abusing the fact that the signal has a wavelength of 2 elements.
    let filter_b = vec![0.99, 0.0, 0.01];
    let decimation_factor = unsafe { NonZeroUsize::new_unchecked(1) };
    // WARN: There is currently no means of exposing generating of filtfilt via
    // `Config::with_config`.
    debug_vhf_conf.stream_fold = {
    ...
    if let StreamFoldOp::Map(Some(map_arg)) = &params_func.op {
        ...
        assert_eq!(
            map_arg.num_before_first_drop,
            filter_b.len().checked_sub(1).unwrap(),
            "The boxcar filter drops exactly 1 less than itself for file writing."
        );
    } else {
        panic!("func.op should have been Map(Some(_))!");
    }
    ...
    // Check the first element is the appropriate time as the time
...
#[test]
#[ignore = "incomplete test"]
fn filtfilt_v2_file_len4_trivial_filter() {
    let debug_vhf_total_len = 12 * VHF_MMAP_WINDOW_LEN;
    const PARAMS_STEP_BY: usize = VHF_MMAP_WINDOW_LEN.checked_sub(1).unwrap();
    // Ideally this should be derived from debug_vhf_conf, but we cannot due to circular
    // requirement; const to enforce at compile time.
    ...
    // This filter is abusing the fact that the signal has a wavelength of 2 elements.
    let filter_b = vec![0.99, 0.1, 0.01, -0.1];
    let decimation_factor = unsafe { NonZeroUsize::new_unchecked(1) };
    // WARN: There is currently no means of exposing generating of filtfilt via
    // `Config::with_config`.
    debug_vhf_conf.stream_fold = {
