...
\begin{tikzpicture}
	% Constants
	\tikzmath{\pagewidth = 1.7; \pageheight=0.8; \textstagger=-0.30;}
	...
	\begin{scope}[shift={(0,2.5*\textstagger-\pageheight)}]
		...
		\begin{scope}[shift={(4*\pagewidth,0)}]
			...
			++(0,-\pageheight) node[anchor=center] at (2.5 * \pagewidth, \pageheight/2) {$\cdots$} ++(\pagewidth,\pageheight)
			++(0,-\pageheight) rectangle ++(\pagewidth,\pageheight);
			\draw[pattern={Lines[angle=-45,distance={3pt}]},pattern color=blue] (4*\pagewidth,0) rectangle +(\pagewidth,\pageheight)
			node [anchor=center, fill=white] at (4.5*\pagewidth, \pageheight/2) {\scriptsize \textbf{End}};
		\end{scope}
	\end{scope}
\end{tikzpicture}
\end{document}
