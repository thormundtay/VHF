...
\begin{tikzpicture}
	...
	\tikzmath{
		...
		\convrowsdraw = int(min(\convrows, 4)); % This is the number of convolution rows to draw
		\datawidth = 0.9; \dataheight=0.9; \textstagger=-0.30;
		\secondconvoffset = 0.6; % Offset specific to second convolution row and further
		\convstagger = 0.09;
		\outy = \textstagger - (\convrowsdraw + 1) * (\dataheight + \convstagger) - \secondconvoffset; % This is the y-coord of bottom of out data
	}
	...
	\begin{scope}
		...
		\foreach \i [ evaluate=\i as \drawcoord using (\i - \numrect - 1) * \datawidth ] in {1, 2, ...,  \totalrect} {
				\draw (\drawcoord, \outy) rectangle ++(\datawidth, \dataheight); % Box for point
			}
		\draw [thick, red!70!blue] ({-(\numrect + 1.5)*\datawidth}, \outy) -- ({(\numrect + 1.5)*\datawidth}, \outy);
	...
	\begin{scope}
				...
				\foreach \convidx [
					evaluate=\convidx as \convx using (\convidx * \datawidth + \startrectx),
						...
						\begin{scope}
							...
							{ \draw [red] (sum.south) -- (sum |-, {\outy+\dataheight}); } % Draw to right for all but last
						\end{scope}
					}
			}
		...
		\draw[red] ({ ( -\numrect + \convrectlen - 1 ) * \datawidth }, \outy) rectangle ++(\datawidth, \dataheight);
	\end{scope}
